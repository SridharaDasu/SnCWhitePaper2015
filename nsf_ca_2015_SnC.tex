...
data. % oblivious of the computing resource provisioning.
...
interruptions, e.g., by relocating jobs on an alternate site, while
...
for agile operations.  The MIT group, which developed DDM, will take
on this task.
...
of objects, and packing them in appropriately sized containers resulted in
a MiniAOD which is less than 50~kB per event. The MiniAOD is now
the main data format that has been used for early results on the 13TeV data from CMS.
%fkw: we already say this in the introduction:
%while niche use cases involving the original AOD format will be
%supported as needed. In rare cases FEVT access may also be needed.  As the
%MiniAOD improves we anticipate AOD replica counts will become small.  
Other activities included a systematic effort to make the core CMSSW
...
And we transitioned the computing infrastructure and services that CMS depends on towards a suite of services that are much more agile
as described in Section~\ref{cis}.
